...
version, unless a newer version made exactly the same errors as the one before
it; any other row is shown from the newest version it was measured on.
...
def table_cells(cells):
    ...
    model and sits beside the old one - unless it made exactly the same errors
    as on the version before it, in which case the update did not change the
    engine and only the newer row is shown. measurements.tsv keeps every cell.
    """
    def key(cell):
    ...
    shown = [cell for cell in cells if chosen[key(cell)] is cell]

    versions = {}
    for cell in shown:
        if is_apple(cell):
            versions.setdefault(key(cell)[:3], []).append(cell)
    unchanged = set()
    for same_row in versions.values():
        same_row.sort(key=lambda c: version_key(c["os"]))
        for older, newer in zip(same_row, same_row[1:]):
            counts = error_counts(older)
            if None not in counts and counts == error_counts(newer):
                unchanged.add(id(older))
    return [cell for cell in shown if id(cell) not in unchanged]


# Two cells with every one of these equal scored the same rows and made the
# same errors. Timing and memory are left out: they differ between any two runs.
# A summary missing one of them cannot show that, and is never collapsed.
ERROR_COUNT_FIELDS = ("rows", "skipped", "reference_words", "reference_characters",
                      "substitutions", "deletions", "insertions", "character_errors")


def error_counts(cell):
    return tuple(cell["summary"].get(field) for field in ERROR_COUNT_FIELDS)
...
def index_document(groups, prov, catalog, missing, extra, wildcard, limits, notes):
    ...
    if len(prov["versions"]) > 1:
        text.append(paragraph(
            ...
            "operating system - except where a newer version made exactly the "
            "same errors as the one before it, which shows only the newer row. "
            "Every other row is shown from the newest version it was measured "
            "on, and measurements.tsv holds every cell."
...
def tsv_document(cells, prov):
    ...
    lines += [
        "#",
        "# measurements.tsv - every cell of the battery. One row per model,",
        "# corpus and macOS version. Generated by tools/battery-report.py; do",
        "# not edit by hand.",
        "#",
